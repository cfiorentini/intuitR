...
let f1 = (Atom "p1") :&: (Atom "p2")
let f2 = (Atom "p3") :|: (Atom "p5") 
let f3 = f1 :=>: f2
oneForm [ Input "f1" Fact f1 , Input "f2" Fact f2 , Input "f3" Conjecture f3  ]
 (((p1 & p2) & (p3 | p5))
     => ((p1 & p2) => (p3 | p5)))

oneForm [ Input "f1" Conjecture f1 , Input "f2" Fact f2 , Input "f3" Fact f3  ]
   (((p3 | p5) & ((p1 & p2) => (p3 | p5)))
           => (p1 & p2))

  

** Exactly one Conjecture **
-}
oneForm fs
  = case un conjs of
      [conj] ->
        case un axs of -- list of Conjecture formulas (assumptions)
          [] -> conj
          ass -> foldr1 (:&:) ass :=>: conj
      _ -> error "Need exactly one conjecture!"
  where
  (axs,conjs) = partition (\ (Input _ r _) -> r == Fact) fs  -- partition (Facts,Conjectures)
  un xs = [ x | Input _ _ x <- xs ]

showFCubeProblem :: [Input Form] -> String
showFCubeProblem fs =
  "intDecide(" ++ showFCube (oneForm fs) ++ ",X,'generated by intuitR')."
